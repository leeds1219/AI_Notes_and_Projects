...
class Linear(FunctionNode):
    ...
    def _backward(gradient, *inputs):
        ...
        "*** YOUR CODE HERE (Q2) ***"
        result1 = np.dot(gradient, inputs[1].T)
        result2 = np.dot(inputs[0].T, gradient)
...
class ReLU(FunctionNode):
    ...
    def _backward(gradient, *inputs):
        ...
        assert gradient.shape == inputs[0].shape
        "*** YOUR CODE HERE (Q3) ***"
        masked_gradient = gradient * np.heaviside(inputs[0], 0)
...
class SquareLoss(FunctionNode):
    ...
    def _backward(gradient, *inputs):
        ...
        result1 = gradient * (inputs[0] - inputs[1]) / size
        result2 = gradient * (inputs[1] - inputs[0]) / size
        return result1, result2
...
class SoftmaxLoss(FunctionNode):
    ...
    def _backward(gradient, *inputs):
        ...
        size = inputs[0].shape[0]  # Batch size

        gradient_adjustment1 = gradient * x_prob / size
        gradient_adjustment2 = gradient * (-1 * log_probs) / size

        return gradient_adjustment1, gradient_adjustment2
